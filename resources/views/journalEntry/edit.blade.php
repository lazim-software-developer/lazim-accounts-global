...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card repeater" data-value='{!! json_encode($journalEntry->accounts->map(function ($v) {
                $v->account = $v->type . '_' . $v->account;
                return $v;
            })) !!}'>
                                    ...
                                    <td width="25%" class="form-group pt-0">
                                        <select name="account" id="account" class="form-control select" required>
                                            @foreach($accounts as $id => $name)
                                                <option value="account_{{ $id }}" selected>{{ $name }}</option>
                                            @endforeach
                                            @foreach($vendors as $id => $name)
                                                <option value="vendor_{{ $id }}">{{ $name }}</option>
                                            @endforeach
                                            @foreach($customers as $id => $name)
                                                <option value="customer_{{ $id }}">{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </td>                                        </div>
