                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group" id="vender-box">
                                ...
                                {{ Form::select('vender_id', $venders, $bill->vender_id ?? null, ['class' => 'form-control select', 'id' => 'vender', 'data-url' => route('bill.vender'), 'required' => 'required']) }}
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {{ Form::date('bill_date', $bill->bill_date ?? date('Y-m-d'), ['class' => 'form-control', 'required' => 'required']) }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('due_date', __('Due Date'), ['class' => 'form-label']) }}
                                        {{ Form::date('due_date', $bill->due_date ?? date('Y-m-d'), ['class' => 'form-control', 'required' => 'required']) }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {{ Form::select('category_id', $category ?? [], $bill->category_id ?? null, ['class' => 'form-control select', 'required' => 'required']) }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('order_number', __('Order Number'), ['class' => 'form-label']) }}
                                        {{ Form::number('order_number', $bill->order_number ?? '', ['class' => 'form-control']) }}
                                    </div>
                                </div>
                ...
                <div class="card-body table-border-style">
                    <div class="table-responsive">
                        <table class="table mb-0" data-repeater-list="services" id="services-table">
                            ...
                            @php
                            $BankProducts=DB::table('bill_products')->where('bill_id', $BankAccount->ref_id)->where('bill_account_id', $BankAccount->id)->orderBy('id', 'asc')->first();
                            @endphp
                                ...
                                <tr class="service-row">
                                        ...
                                        <select name="chart_account_id" class="form-control account-select" required>
                                            ...
                                            @if(!empty($chartAccounts))
                                                @foreach ($chartAccounts as $key => $chartAccount)
                                                    <option value="{{ $key }}" {{ $key == $BankAccount->chart_account_id ? 'selected' : '' }}>
                                                        {{ $chartAccount }}
                                                    </option>
                                                    @if(!empty($subAccounts))
                                                        @foreach ($subAccounts as $subAccount)
                                                            @if ($key == $subAccount['account'])
                                                                <option value="{{ $subAccount['id'] }}" class="ms-3" {{ $subAccount['id'] == $BankAccount->chart_account_id ? 'selected' : '' }}>
                                                                    &nbsp;&nbsp;&nbsp;{{ $subAccount['name'] }}
                                                                </option>
                                                            @endif
                                                        @endforeach
                                    ...
                                    <td class="form-group">
                                        <div class="input-group">
                                            {{ Form::text('amount', $BankAccount->price, ['class' => 'form-control amount-input', 'placeholder' => __('Amount'), 'required' => 'required']) }}
                                    ...
                                    <!-- Description -->
                                    <td class="form-group">
                                        {{ Form::textarea('description', $BankAccount->description, ['class' => 'form-control item-description', 'rows' => '2', 'placeholder' => __('Description')]) }}
                                    </td>
                                    <!-- Item Selection -->
                                    <td class="form-group">
                                        {{ Form::select('item_id', $product_services ?? [], $BankProducts ? $BankProducts->product_id : null, ['class' => 'form-control item-select', 'data-url' => route('bill.product'), 'placeholder' => __('Select Item'), 'selected' => $BankProducts ? 'selected' : '']) }}
                                    </td>
                                    ...
                                    <td class="form-group">
                                        <div class="input-group">
                                            {{ Form::number('unit_price',$BankProducts ? $BankProducts->price : '0', ['class' => 'form-control unit-price', 'step' => '0.01', 'placeholder' => __('Price'), 'required' => 'required','readonly' => 'readonly']) }}
                                            <span class="input-group-text">{{ \Auth::user()->currencySymbol() ?? '$' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="form-group">
                                        <div class="input-group">
                                            {{ Form::number('discount',$BankProducts ? $BankProducts->discount : '0', ['class' => 'form-control discount', 'step' => '0.01', 'placeholder' => __('Discount')]) }}
                                            <span class="input-group-text">{{ \Auth::user()->currencySymbol() ?? '$' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <!-- Tax -->
                                    <td class="form-group">
                                        <div class="taxes-display">@if($BankAccount->vat_amount > 0)
                                        <span class="badge bg-primary mt-1 mr-2">VAT (5%)</span>
                                        @else
                                        <span class="badge bg-primary mt-1 mr-2">NO VAT (0%)</span>
                                        @endif</div>
                                        {{ Form::hidden('tax_ids', '', ['class' => 'tax-ids']) }}
                                        {{ Form::hidden('tax_amount', $BankAccount ? $BankAccount->vat_amount : '0', ['class' => 'tax-amount']) }}
                                        {{ Form::hidden('tax_rate', $BankProducts ? $BankProducts->tax : '0', ['class' => 'tax-rate']) }}
                                    </td>
                                    
                                    <!-- Total Amount -->
                                    <td class="text-end">
                                        <span class="row-total">{{ $BankAccount ? $BankAccount->total_amount : '0.00' }}</span>
                                    </td>
