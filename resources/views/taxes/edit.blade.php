...
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group col-md-6">
            {{ Form::label('account_id', __('Tax Account'),['class'=>'form-label']) }}
            <select name="account_id" id="account_id" class="form-control select" required>
                @foreach($accounts as $id => $name)
                    <option value="{{ $id }}" @if($id == $tax->account_id) selected @endif>{{ $name }}</option>
                @endforeach
            </select>
            @error('account_id')
            <small class="invalid-account_id" role="alert">
                <strong class="text-danger">{{ $message }}</strong>
            </small>
            @enderror
        </div>
