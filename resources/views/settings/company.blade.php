...
@section('content')
                    ...
                    <div id="useradd-2" class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    @php
                                    $categoryData = App\Models\ProductServiceCategory::where('created_by', Auth::user()->creatorId())->where('type', 'income')->get();
                                    $Services=App\Models\ProductService::where('created_by', '=', Auth::user()->creatorId())->where('category_id', '=', $categoryData->pluck('id'))->get();
                                    @endphp
                                    {{ Form::label('invoice_service_charge', __('Invoice Service Charge'), ['class' => 'form-label']) }}

                                    @if($Services)
                                        {{ Form::select('invoice_service_charge', 
                                            ['' => 'Select Service'] + $Services->pluck('name', 'id')->toArray(), 
                                            null, 
                                    ...
                                    @else
                                        {{ Form::select('invoice_service_charge', 
                                            ...
                                            null, 
                                            ['class' => 'form-control selectric', 'disabled' => 'disabled']) }}
                                    @endif
                                    
                                    @error('invoice_service_charge')
                                        <span class="invalid-invoice_service_charge" role="alert">
                                            <strong class="text-danger">{{ $message }}</strong>
                                        </span>
                                    @enderror
                                </div>
                                <div class="form-group col-md-6">
                                    {{ Form::label('vat_charge', __('VAT Charge'), ['class' => 'form-label']) }}

                                    @php
                                        $types = App\Models\ChartOfAccountType::where('created_by', '=', Auth::user()->creatorId())->where('name', '=', 'Liabilities')->whereOr('name', '=', 'liabilities')->first();
                                        $Services=App\Models\ChartOfAccount::where('created_by', '=', Auth::user()->creatorId())->where('type', '=', $types->id)->get();
                                    @endphp

                                    @if($Services)
                                        {{ Form::select('vat_charge', 
                                            ['' => 'Select VAT'] + $Services->pluck('name', 'id')->toArray(), 
                                            null, 
                                            ['class' => 'form-control selectric']) }}
                                    @else
                                        {{ Form::select('vat_charge', 
                                    ...
                                    @endif
                                    @error('vat_charge')
                                        <span class="invalid-invoice_vat_charge" role="alert">
                                            <strong class="text-danger">{{ $message }}</strong>
                                        </span>
                                    @enderror
                                </div>
                                            ...
                                            <div class="list-group">
                                                <div class="list-group-item form-switch form-switch-right">
                                                    ...
                                                    <input class="form-check-input" name='{{ $EmailTemplate->id }}'
                                                        ...
                                                        data-url="{{ route('status.email.language', [$EmailTemplate->template->id]) }}" @endif />
                                                    <label class="form-check-label"@if ($EmailTemplate->template)
                                                        for="email_tempalte_{{ $EmailTemplate->template->id }}" @endif></label>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                    ...
                    {{-- <div class="" id="useradd-11">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <a href="javascript:void(0)" data-size="md" data-url="{{ route('webhook.create') }}"
                                    data-ajax-popup="true" data-bs-toggle="tooltip"
                                    title="{{ __('Create New webhook') }}" class="btn btn-sm btn-primary">
    ...
    @push('css-page')
        <link rel="stylesheet"
            href=" {{ Module::asset('LandingPage:Resources/assets/css/summernote/summernote-bs4.css') }}" />
    @endpush
    @push('script-page')
        <script src="{{ Module::asset('LandingPage:Resources/assets/js/plugins/summernote-bs4.js') }}"
