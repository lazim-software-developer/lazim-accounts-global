...
@section('action-btn')
    <div class="float-end">
        ...
        @endphp
        {{-- @if ($CurrentBuilding)
        <a href="#" data-size="md" data-bs-toggle="tooltip" title="{{ __('Sync Invoices') }}"
        data-url="{{ route('customer.sync.bulkinvoice') }}" data-ajax-popup="true"
        data-title="{{ __('Sync Invoices') }}" class="btn btn-sm btn-primary"> 
            {{ __('Sync Invoices') }}
        </a>
        @endif  --}}
        <a href="#" data-size="md" data-bs-toggle="tooltip" title="{{ __('Import') }}"
            data-url="{{ route('invoice.file.import') }}" data-ajax-popup="true"
            data-title="{{ __('Import invoice CSV file') }}" class="btn btn-sm btn-primary">
            {{ __('Import Invoice') }}
        </a>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row d-flex align-items-center justify-content-end">
                            ...
                            @if (!\Auth::guard('customer')->check())
                                <div class="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12 mr-2">
                                    <div class="btn-box">
                                        {{ Form::label('customer', __('Owner'), ['class' => 'text-type']) }}
                            ...
                            <tbody>
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        ...
                                        {{-- <td>{{ !empty($invoice->ref_number) ? $invoice->ref_number : '' }}</td> --}}
                                        @if (!\Auth::guard('customer')->check())
                                        ...
                                        <td>
                                            @php
                                                $property_number = DB::connection('mysql_lazim')
                                                    ->table('flats')
                                                    ->find($invoice->flat_id)?->property_number;
                                            @endphp
                                            {{ $property_number }}
                                        </td>
                                        @if (Gate::check('edit invoice') || Gate::check('delete invoice') || Gate::check('show invoice'))
