@php
    ...
    $buildingIds = DB::connection('mysql_lazim')
        ->table('building_owner_association')
        ->where('owner_association_id', $users->currentOA())
        ->pluck('building_id');
    $buildings = DB::connection('mysql_lazim')->table('buildings')->whereIn('id', $buildingIds)->get();
    $currentBuildingObj = $buildings->firstWhere('id', $users->building_id);
    $userBuilding = \App\Models\User::whereNotNull('building_id')
        ->where('type', 'building')
        ->pluck('id', 'building_id');
@endphp
        ...
        <div class="me-auto dash-mob-drp">
            <ul class="list-unstyled">
                ...
                @if (Gate::check('create product & service') ||
                    ...
                    <li class="ml-2 dropdown dash-h-item">
                            ...
                            @endif --}}
                                @if (Gate::check('create customer'))
                                    <a class="dropdown-item" href="#" data-size="lg"
                                        ...
                                        data-title="{{ __('Create New Resident') }}"><i
                                            class="ti ti-user"></i>{{ __('Create New Resident') }}</a>
                                @endif
                ...
                @impersonating($guard = null)
                    <li class="dropdown dash-h-item drp-company">
                        <a class="btn btn-danger btn-sm me-3" href="{{ route('exit.company') }}"><i class="ti ti-ban"></i>
                            {{ __('Exit ') }}{{ \Auth::user()->name }}{{ __(' Login ') }}
                        </a>
                    </li>
                @endImpersonating
                    ...
                    <div class="dropdown-menu dash-h-dropdown dropdown-menu-end">
                        ...
                        @if (\Auth::user()->type == 'super admin')
                            ...
                            <a class="dropdown-item text-primary"
                                href="{{ route('manage.language', [$currantLang]) }}">
                                {{ __('Manage Language') }}
                            </a>
                        @endif
