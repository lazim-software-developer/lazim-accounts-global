...
class NotificationTemplatesController extends Controller
{
    public function index($id = null, $lang = 'en')
    {
        ...
        // {        
            $query = NotificationTemplates::whereNotIn('name', ['New Customer', 'New Vendor', 'New Proposal']);

            if($id != null)
            {
                $notification_template     = $query->where('id', $id)->first();
            }
            else
            {
            ...
            $notification_templates = NotificationTemplates::whereNotIn('name', ['New Customer', 'New Vendor', 'New Proposal'])->get();

            return view('notification_templates.index', compact('notification_template','curr_noti_tempLang','languages','notification_templates'));
        // }
        // else
        // {
