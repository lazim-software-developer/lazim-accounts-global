...
class BankAccountController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create bank account')) {
            ...
            $chartAccounts = ChartOfAccount::select([\DB::raw('CONCAT(code, " - ", name) AS code_name'), 'id'])->where('created_by', \Auth::user()->creatorId())->get()->pluck('code_name', 'id')->prepend('Select Account', 0);
        ...
        if (\Auth::user()->can('edit bank account')) {
            if ($bankAccount->created_by == \Auth::user()->creatorId()) {
                ...
                $chartAccounts = ChartOfAccount::select([
                    ...
                    'id'
                ])->where('created_by', \Auth::user()->creatorId())->get()->pluck('code_name', 'id')->prepend('Select Account', 0);
        ...
        if (\Auth::user()->can('delete bank account')) {
            if ($bankAccount->created_by == \Auth::user()->creatorId()) {
                ...
                if (!empty($revenue) && !empty($invoicePayment) && !empty($transaction) && !empty($payment) && !empty($billPayment)) {
                    ...
                    TransactionLines::deleteAndRecalculateTransactionBalance($bankAccount, 'Bank Account');
                    $bankAccount->delete();
