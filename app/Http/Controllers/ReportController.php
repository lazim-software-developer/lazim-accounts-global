...
class ReportController extends Controller
{
    ...
    public function incomeVsExpenseSummaryNew(Request $request)
    {
        if (\Auth::user()->can('income vs expense report')) {
            ...
            // main query
            $rawData = \DB::table('chart_of_accounts as coa')
                ->select(
                    'coa.id',
                    'coa.name as account_name',
                    'subtype.name as subtype_name',
                    \DB::raw('IFNULL(SUM(tl.closing_balance - tl.opening_balance), 0) as balance_difference')
                )
                // ->leftJoin('transaction_lines as tl', function ($join) {
                //     $join->on('coa.id', '=', 'tl.account_id')
                //         ->whereBetween('tl.date', ['2025-01-01', '2025-01-20']);
                // })
                ->leftJoin('transaction_lines as tl', function ($join) use ($start, $end) { // Pass $start and $end here
                    $join->on('coa.id', '=', 'tl.account_id')
                        ->whereBetween('tl.date', [$start, $end]);
                })
                ->join('chart_of_account_sub_types as subtype', 'coa.sub_type', '=', 'subtype.id')
                ->join('chart_of_account_types as type', 'subtype.type', '=', 'type.id')
                ->where('coa.building_id', 1)
                ->whereIn('coa.type', [3, 4])
                ->groupBy('coa.id', 'coa.name', 'subtype.name')
                ->get();

            // Group data by `subtype_name` and calculate totals
            $reportData = $rawData->groupBy('subtype_name')->map(function ($accounts, $subtypeName) {
                $groupTotal = $accounts->sum('balance_difference');
            ...
            $grandTotal = $reportData->sum('group_total');
            $filter['start_date'] = $start;
            $filter['end_date'] = $end;

            return view('report.income_vs_expense_summary_new', compact('filter', 'reportData', 'grandTotal'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function taxSummary(Request $request)
    {
        ...
        if (\Auth::user()->can('tax report')) {
            ...
            $fromDate = date('Y-m-d H:i:s', strtotime($request->from ?: '-1 month'));
            $toDate = date('Y-m-d H:i:s', strtotime($request->to ?: 'now'));
            $type = $request->type;
            ...
            $invoices = Invoice::select('invoices.*')->where('invoices.created_by', '=', \Auth::user()->creatorId())
                ->whereBetween('invoices.created_at', [$fromDate, $toDate])->get();
                    ...
                    if (!empty($item->tax)) {
                        ...
                        foreach ($incomeTaxes as $taxe) {
                            ...
                            if (!isset($incomeTaxesData[!empty($taxe) ? ($taxe->name) : ''])) {
                                $incomeTaxesData[!empty($taxe) ? ($taxe->name) : '']['taxableAmount'] = 0;
                                $incomeTaxesData[!empty($taxe) ? ($taxe->name) : '']['tax'] = 0;
                            }
                            $incomeTaxesData[!empty($taxe) ? ($taxe->name) : '']['tax'] += $taxDataPrice;
                            $incomeTaxesData[!empty($taxe) ? ($taxe->name) : '']['taxableAmount'] += ($item->price * $item->quantity);
                        }
                    }
                }
            }

            $bills = Bill::select('bills.*')->where('bills.created_by', '=', \Auth::user()->creatorId())
                ->whereBetween('bills.created_at', [$fromDate, $toDate])->get();
                    ...
                    if (!empty($item->tax)) {
                        ...
                        foreach ($expenseTaxes as $taxe) {
                            ...
                            if (!isset($expenseTaxesData[!empty($taxe) ? ($taxe->name) : ''])) {
                                $expenseTaxesData[!empty($taxe) ? ($taxe->name) : '']['taxableAmount'] = 0;
                                $expenseTaxesData[!empty($taxe) ? ($taxe->name) : '']['tax'] = 0;
                            }
                            $expenseTaxesData[!empty($taxe) ? ($taxe->name) : '']['tax'] += $taxDataPrice;
                            $expenseTaxesData[!empty($taxe) ? ($taxe->name) : '']['taxableAmount'] += ($item->price * $item->quantity);
                        }
                    }
                }
            }
            ...
            $filter['startDateRange'] = $fromDate;
            $filter['endDateRange'] = $toDate;
            ...
            return view('report.tax_summary_new', compact('filter', 'expenseTaxesData', 'incomeTaxesData', 'settings', 'img'), $data);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
        ...
        if (\Auth::user()->can('ledger report')) {
            ...
            if ($request->account) {
                if (strpos($request->account, '_') === false) {
                    [$type, $accountId] = ['account', $request->account];
                } else {
                    [$type, $accountId] = explode('_', $request->account);
                }
            }
            if (!empty($request->start_date) && !empty($request->end_date)) {
                $start = $request->start_date;
            ...
            if ($type == 'account' && !empty($accountId)) {
                $chart_accountCheck = ChartOfAccount::where('id', $accountId)->where('created_by', \Auth::user()->creatorId())->first();
                if ($chart_accountCheck->name == 'Sundry Creditors') {
                    $type = 'vendor';
                    $accountId = null;
                } elseif ($chart_accountCheck->name == 'Sundry Debtors') {
                    $type = 'customer';
                    $accountId = null;
                }
            }
            ...
            return view('report.ledger_summary', compact('filter', 'accounts', 'type', 'accountId', 'customers', 'vendors', 'subAccounts', 'chart_accounts'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function trialBalanceSummary(Request $request, $view = 'expand')
    {
        if (\Auth::user()->can('trial balance report')) {
            ...
            foreach ($types as $type) {
                ...
                if ($parentAccounts->isNotEmpty()) {
                    foreach ($parentAccounts as $parentAccount) {
                        ...
                        $parentAccountArray = [];
                        $parentAccountArrayTotal = [];
                        $startDate = \DB::getPdo()->quote($start); // Escape $start
                        $endDate = \DB::getPdo()->quote($end);    // Escape $end

                        $parentAccs = TransactionLines::select(
                            'chart_of_accounts.id',
                            'chart_of_accounts.code',
                            'chart_of_accounts.name',
                            \DB::raw('sum(debit) as totalDebit'),
                            \DB::raw('sum(credit) as totalCredit'),
                            // Subquery to get the opening balance
                            \DB::raw('(
                        SELECT opening_balance
                        FROM transaction_lines AS tl
                        WHERE tl.account_id = transaction_lines.account_id
                        AND tl.date >= ' . $startDate . '
                        ORDER BY tl.date ASC, tl.id ASC
                        LIMIT 1
                        ) as opening_balance'),
                            // Subquery to get the closing balance
                            \DB::raw('(
                        ...
                        $accounts = TransactionLines::select(
                            'chart_of_accounts.id',
                            'chart_of_accounts.code',
                            'chart_of_accounts.name',
                            \DB::raw('sum(debit) as totalDebit'),
                        ...
                        if ($parentAccs == [] && $accounts != []) {
                            ...
                            $parentAccs = ChartOfAccount::select('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name', \DB::raw('0 as totalDebit'), \DB::raw('0 as totalCredit'), \DB::raw('0 as opening_balance'), \DB::raw('0 as closing_balance'));
                            $parentAccs->leftjoin('chart_of_account_parents', 'chart_of_accounts.id', 'chart_of_account_parents.account');
                            $parentAccs->where('chart_of_accounts.type', $type->id);
                            $parentAccs->where('chart_of_accounts.name', $parentAccount->name);
                            $parentAccs->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
                        ...
                        $parenttotalBalance = 0;
                        $parentOpeningBalance = 0;
                        $parenntClosingBalance = 0;
                        $parentcreditTotal = 0;
                        $parenntdebitTotal = 0;
                        $parenttotalAmount = 0;

                        foreach ($parentAccs as $k => $account) {
                            $Balance = $account['totalCredit'] - $account['totalDebit'];
                            $parenttotalBalance += $Balance;
                        ...
                        if (!empty($parentAccountArray)) {
                            ...
                            $parentOpeningBalance = $parentAccountArray[0]['opening_balance'];
                            $parentClosingBalance = end($parentAccountArray)['closing_balance'];
                            ...
                            $dataTotal = [
                                ...
                                'totalCredit' => $parentcreditTotal,
                                'totalDebit' => $parenntdebitTotal,
                                'opening_balance' => $parentOpeningBalance,
                                'closing_balance' => $parentClosingBalance,
                            ];
                ...
                if ($totalParentAccountArray != []) {
                    ...
                    $accounts = TransactionLines::select(
                        ...
                        ->where('transaction_lines.date', '>=', $start)
                        ->where('transaction_lines.date', '<=', $end)
                        ->groupBy('account_id')
                        ->get()
                        ->toArray();
                } else {
                ...
                foreach ($entries as $entry) {
                    ...
                    if (!isset($totalAccounts[$category][$name])) {
                        $totalAccounts[$category][$name] = [
                            ...
                            'totalDebit' => 0,
                            'totalCredit' => 0,
                            'opening_balance' => isset($entry['opening_balance']) ? $entry['opening_balance'] : '',
                            'closing_balance' => isset($entry['closing_balance']) ? $entry['closing_balance'] : '',
                        ];
                    }
            ...
            $vendorTransactionLineQuery = StakeholderTransactionLine::whereNotNull('vender_id')
                ->where('created_by', \Auth::user()->creatorId())
                ->whereBetween('date', [$start, $end]);

            $sundryCreditors = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
                ->where('name', 'Sundry Creditors')->first();

            if ($vendorTransactionLineQuery->count() > 0) {
                // $totalAccounts[$sundryCreditors->types->name][$sundryCreditors->subType->name] = [
                //     'account_id' => $sundryCreditors->subType->id,
                //     'account_code' => $sundryCreditors->subType->code,
                //     'account_name' => $sundryCreditors->subType->name,
                //     'account' => 'parent',
                //     'totalDebit' => $vendorTransactionLineQuery->sum('debit'),
                //     'totalCredit' => $vendorTransactionLineQuery->sum('credit'),
                // ];

                // Clone the query for opening balance
                $openingBalanceQuery = clone $vendorTransactionLineQuery;
                $opening_balance = $openingBalanceQuery
                    ->orderBy('date', 'asc') // Ensure the earliest date comes first
                    ->orderBy('id', 'asc') // Secondary ordering for tie-breaker
                    ->first()->opening_balance ?? 0; // Default to 0 if no data

                // Clone the query for closing balance
                $closingBalanceQuery = clone $vendorTransactionLineQuery;
                $closing_balance = $closingBalanceQuery
                    ->orderBy('date', 'desc') // Ensure the latest date comes first
                    ->orderBy('id', 'desc') // Secondary ordering for tie-breaker
                    ->first()->closing_balance ?? 0; // Default to 0 if no data

                $totalAccounts[$sundryCreditors->types->name][$sundryCreditors->name] = [
                    'account_id' => $sundryCreditors->id,
                    'account_code' => $sundryCreditors->code,
                    'account_name' => $sundryCreditors->name,
                    'account' => '',
                    'totalDebit' => $vendorTransactionLineQuery->sum('debit'),
                    'totalCredit' => $vendorTransactionLineQuery->sum('credit'),
            ...
            $customerTransactionLineQuery = StakeholderTransactionLine::whereNotNull('customer_id')
                ->where('created_by', \Auth::user()->creatorId())
                ->whereBetween('date', [$start, $end]);

            $sundryDebitors = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
                ->where('name', 'Sundry Debtors')->first();

            if ($customerTransactionLineQuery->count() > 0) {
                // $totalAccounts[$sundryDebitors->types->name][$sundryDebitors->subType->name] = [
                //     'account_id' => $sundryDebitors->subType->id,
                //     'account_code' => $sundryDebitors->subType->code,
                //     'account_name' => $sundryDebitors->subType->name,
                //     'account' => 'parent',
                //     'totalDebit' => $customerTransactionLineQuery->sum('debit'),
                //     'totalCredit' => $customerTransactionLineQuery->sum('credit'),
                // ];

                // Clone the query for opening balance
                $openingBalanceQuery = clone $customerTransactionLineQuery;
                $opening_balance = $openingBalanceQuery
                    ->orderBy('date', 'asc') // Ensure the earliest date comes first
                    ->orderBy('id', 'asc') // Secondary ordering for tie-breaker
                    ->first()->opening_balance ?? 0; // Default to 0 if no data
                ...
                // Clone the query for closing balance
                $closingBalanceQuery = clone $customerTransactionLineQuery;
                $closing_balance = $closingBalanceQuery
                    ->orderBy('date', 'desc') // Ensure the latest date comes first
                    ->orderBy('id', 'desc') // Secondary ordering for tie-breaker
                    ->first()->closing_balance ?? 0; // Default to 0 if no data

                $totalAccounts[$sundryDebitors->types->name][$sundryDebitors->name] = [
                    'account_id' => $sundryDebitors->id,
                    'account_code' => $sundryDebitors->code,
                    'account_name' => $sundryDebitors->name,
                    'account' => '',
                    'totalDebit' => $customerTransactionLineQuery->sum('debit'),
                    'totalCredit' => $customerTransactionLineQuery->sum('credit'),
                    'opening_balance' => $opening_balance,
    ...
    public function getAgingReport()
    {
        ...
        $reportData = $connection->table('aging_reports')
            ...
            ->where('aging_reports.building_id', $currentBuildId)
            ->orderBy('aging_reports.created_at', 'desc')
            ->get([
        ...
        return view('report.aging_report', compact('reportData'));
    }
}
