...
class Customer extends Authenticatable
{
    ...
    public function customerReceipt($customerId)
    {
        $receipts = Revenue::whereJsonContains('customer_id', (string)$customerId)
            ->orderBy('date', 'desc')
            ->get();
    ...
    public function customerTotaldue($customerId)
    {
        $dueInvoices = Invoice:: where('customer_id', $customerId)->whereNotIn(
            'status', [
                        '0',
    ...
    public function customerTotalInvoiceYearSum($customerId)
    {
        $currentYear = Carbon::now()->year;

        $invoices = Invoice:: where('customer_id', $customerId)->whereYear('issue_date', $currentYear)->get();
        $total    = 0;
        foreach($invoices as $invoice)
        {
            $total += $invoice->getTotal();
    ...
    public function customerTotalReceiptAmountYearSum($customerId)
    {
        $currentYear = Carbon::now()->year;

        $receipts = Revenue:: where('customer_id', $customerId)->whereYear('date', $currentYear)->get();
        $total    = 0;
    ...
    public function customerTotalInvoiceCurrentYear($customerId)
    {
        $invoices = Invoice:: where('customer_id', $customerId)->whereYear('issue_date',date('Y'))->count();
    ...
    {
        return StakeholderTransactionLine::where('customer_id', $this->id)->orderBy('id', 'desc')->first()
        ? StakeholderTransactionLine::where('customer_id', $this->id)->orderBy('id', 'desc')->first()->closing_balance
        : 0;
    }
}
